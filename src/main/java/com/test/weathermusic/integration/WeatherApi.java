...
import com.test.weathermusic.util.bean.CacheHelper;
...
public class WeatherApi {
    ...
    public String getTemperature(PlaylistParamsDto params) {
        final MultiValueMap<String, String> paramMap = buildMapFromParams(params);
        JsonNode response = restTemplate.getForObject(
                URIBuilder.buildWithParams(WEATHER_URI + WEATHER_PATH, paramMap),
                JsonNode.class);
        return response.findValue("temp").asText();
    }

    String tempFromCache(PlaylistParamsDto params) {
        LOGGER.warn("Weather API call failed for {}, trying the cache", params);
        return cacheHelper.get("weather", params, String.class);
    }

    MultiValueMap<String, String> buildMapFromParams(PlaylistParamsDto params) {
